...
注意：**在使用相关寄存器之前，应首先使能AFIO的时钟！**

**AFIO_EXTICRx**（外部中断线配置寄存器）：	

​	该寄存器用于配置外部中断`EXTI`线具体应该由哪个端口对应引脚触发。**`STM32`共有16个外部中断线**，分别对应`GPIO`的引脚0到引脚15.而**每个`EXTI`线可以在众多端口（A，B，C，D，E等）中选择一个进行触发**。以上二者结合便可唯一确定一位引脚是否会触发中断。

​	例如将`EXTI5`中断线配置到`GPIO`端口B，则`PB5`引脚在满足中断条件时将申请中断。而**AFIO_EXTICRx**寄存器正是用来实现此操作的。

​	**EXTICR**是一组寄存器，共有四个（**EXTICRx**，x = 1,2,3,4）。每个寄存器的高16位保留，低16位每4位为一个中断线。4个寄存器的低16位均分16个`EXTI`外部中断线。

<img src="../Pic/AFIO1.png" style="zoom:67%;" />

如上图为`EXTICR1`的结构配置图，分别对应的中断线为`EXTI0`-`EXTI3`。`EXTICR2`同理为`EXTI4`-`EXTI7`，以此类推，此处不再列举。
...
​	该寄存器**用于配置一些外设（如USART，TIM等）的复用功能的映射**。其结构图如下：

<img src="../Pic/AFIO2.png" style="zoom:67%;" />

可以看到每两位或一位控制一个外设的映射功能，对于有多种映射模式选择的外设则采用两位进行控制。此处我们以通用定时器`TIM2`来作为示例。

![](../Pic/EXAMPLE.png)

​	可以看到`TIM2`的1~4通道分别复用在了引脚`PA0~3`上。

![](../Pic/EXAMPLE2.png)

结合上面所有引脚定义图，可以发现`TIM2`的通道1-2被重映射到了`PA15`与`PB3`上面，同时3-4通道被映射到引脚了`PB10`，`PB11`上。

因而在重映射寄存器`MAPR`中，对于`TIM2`的映射便可以有如下多种配置模式：

- `00`：默认复位值，无映射。
- `01`：部分映射，将通道1与通道2映射到`PA15`，`PB3`上面，而通道3-4保持不变（`PA2`，`PA3`）。
- `10`：部分映射，通道1-2保持不变（`PA0`，`PA1`）。通道3-4映射到`PB10`，`PB11`上面。
- `11`：完全映射，所有通道全部映射。通道1-4 -> `PA15`，`PB3`，`PB10`，`PB11`.

不是每个外设都有多个映射模式，具体参考对应的芯片数据手册。
